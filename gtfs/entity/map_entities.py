...
def table_def_from_entity(entity_class, metadata):
    ...
    for field_name, field_type in entity_class.FIELDS:
        if issubclass(field_type, types.ForeignKey):
            ...
            columns.append(
                sqlalchemy.Column(field_name, sqlalchemy.String, sqlalchemy.ForeignKey(foreign_key_column_name),
                                  index=True))
        else:
...
def create_and_map_tables(metadata):
    ...
    sqlalchemy.orm.mapper(Transfer, transfers_table, properties={
    "from_stop": relationship(Stop, primaryjoin=transfers_table.c.from_stop_id == stops_table.c.stop_id,
